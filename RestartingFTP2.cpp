...
IMPLEMENT_DYNAMIC_CLASS(RestartingFTP, wxProtocol)
IMPLEMENT_PROTOCOL(RestartingFTP, wxT("ftp"), wxT("ftp"), true)
...
bool RestartingFTP::Connect(wxSockAddress& addr, bool WXUNUSED(wait))
{
...
bool RestartingFTP::Connect(const wxString& host)
{
    wxIPV4address addr;
    addr.Hostname(host);
...
char RestartingFTP::SendCommand(const wxString& command)
{
    if ( m_streaming )
    {
        m_lastError = wxPROTO_STREAMING;
...
bool RestartingFTP::SetTransferMode(TransferMode transferMode)
{
...
bool RestartingFTP::DoSimpleCommand(const wxChar *command, const wxString& arg)
{
...
bool RestartingFTP::ChDir(const wxString& dir)
{
...
bool RestartingFTP::MkDir(const wxString& dir)
{
    return DoSimpleCommand(_T("MKD"), dir);
}

bool RestartingFTP::RmDir(const wxString& dir)
{
    return DoSimpleCommand(_T("RMD"), dir);
}

wxString RestartingFTP::Pwd()
{
    wxString path;
...
bool RestartingFTP::Rename(const wxString& src, const wxString& dst)
{
    wxString str;
...
bool RestartingFTP::RmFile(const wxString& path)
{
    wxString str;
    str = wxT("DELE ") + path;
...
{
public:
    wxInputFTPStream(RestartingFTP *ftp, wxSocketBase *sock)
...
{
public:
    wxOutputFTPStream(RestartingFTP *ftp_clt, wxSocketBase *sock)
...
void RestartingFTP::SetDefaultTimeout(wxUint32 Value)
{
...
wxSocketBase *RestartingFTP::GetPort()
{
    /*
...
wxSocketBase *RestartingFTP::AcceptIfActive(wxSocketBase *sock)
{
    if ( m_bPassive )
        return sock;
...
wxString RestartingFTP::GetPortCmdArgument(const wxIPV4address& addrLocal,
...
wxSocketBase *RestartingFTP::GetActivePort()
{
...
wxSocketBase *RestartingFTP::GetPassivePort()
{
...
wxInputStream *RestartingFTP::GetInputStream(const wxString& path)
{
...
wxOutputStream *RestartingFTP::GetOutputStream(const wxString& path)
{
    if ( ( m_currentTransfermode == NONE ) && !SetTransferMode(BINARY) )
        return NULL;
...
bool RestartingFTP::GetList(wxArrayString& files,
...
bool RestartingFTP::FileExists(const wxString& fileName)
{
...
int RestartingFTP::GetFileSize(const wxString& fileName)
{
...
wxInputStream *RestartingFTP::GetInputStreamRestart(const wxString& path,int startPos)
{
	...
	wxString tmp_str;
	// support re-receiving
	tmp_str.Printf("REST %i",startPos);
	if (!CheckCommand(tmp_str,'3'))
	{
		return NULL;
	}

	tmp_str = wxT("RETR ") + wxURI::Unescape(path);
	if (!CheckCommand(tmp_str, '1'))
	{
		return NULL;
	}
